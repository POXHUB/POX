...
local function closest_larp(max_range: number, handle: BasePart)
    local closest_part, closest_distance = nil, max_range;
    ...
    if (tool) and (tool:FindFirstChild("Handle")) and (now - last_ran > ragebot_delay) then
        ...
        if (ammo) and (range) and (shooting_cooldown) then
            ...
            if (ammo.Value > 0) and (not local_player.Character.BodyEffects["K.O"].Value) and (not local_player.Character.BodyEffects["Reload"].Value) then
                local closest = closest_larp(range.Value, tool.Handle);
                if (closest) then
                    setthreadidentity(2);
                    ...
                    ragebot_delay = shooting_cooldown.Value;
                    last_ran = now;
                end
            end
        end
